package identify_promoter::identify_promoterClient;
...
identify_promoter::identify_promoterClient
...
sub new
{
    ...
    my $self = {
	client => identify_promoter::identify_promoterClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
# Authentication: required
    ...
    {
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function get_promoter_for_gene (received $n, expecting 1)");
    }
    {
	my($params) = @args;
	...
	my @_bad_arguments;
        (ref($params) eq 'HASH') or push(@_bad_arguments, "Invalid type for argument 1 \"params\" (value was \"$params\")");
        if (@_bad_arguments) {
	    my $msg = "Invalid arguments passed to get_promoter_for_gene:\n" . join("", map { "\t$_\n" } @_bad_arguments);
	    Bio::KBase::Exceptions::ArgumentValidationError->throw(error => $msg,
								   method_name => 'get_promoter_for_gene');
	}
    }

    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "identify_promoter.get_promoter_for_gene",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method get_promoter_for_gene",
					    status_line => $self->{client}->status_line,
					    method_name => 'get_promoter_for_gene',
				       );
    }
...
sub status
{
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
        method => "identify_promoter.status",
        params => \@args,
    });
    if ($result) {
        if ($result->is_error) {
...
sub version {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "identify_promoter.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(
            error => "Error invoking method get_promoter_for_gene",
            status_line => $self->{client}->status_line,
            method_name => 'get_promoter_for_gene',
        );
    }
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for identify_promoter::identify_promoterClient\n";
    }
    if ($sMajor == 0) {
        warn "identify_promoter::identify_promoterClient version is $svr_version. API subject to change.\n";
    }
}
...
=head2 get_promoter_for_gene_input
...
=head2 get_promoter_for_gene_output_params
...
package identify_promoter::identify_promoterClient::RpcClient;
